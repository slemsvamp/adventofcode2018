...
namespace day15_beverage_bandits {
    class Part01 {
        ...
        public static void Run() {
            //RunOnce(false, "mockInput-1.txt", 37, 982, 36334);
            //RunOnce(false, "mockInput-2.txt", 46, 859, 39514);
            //RunOnce(false, "mockInput-3.txt", 35, 793, 27755);
            //RunOnce(false, "mockInput-4.txt", 54, 536, 28944);
            //RunOnce(false, "mockInput-5.txt", 20, 937, 18740);
            RunOnce(true, "input.txt", 0, 0, 0);
        }

        public static void RunOnce(bool pDebug, string pFile, int pExpectedRounds, int pExpectedHitpoints, int pExpectedTotal) {
            // 237510, too high
            // 236430, too high
            // 235672, too high
            // 201991, not right
            ...
            for (int y = 0; y < lines.Length; y++) {
                for (int x = 0; x < lines[y].Length; x++) {
                    ...
                    if (lines[y][x] == 'G') {
                        ...
                        var elf = new Entity { Race = Race.Elf, HitPoints = 200, Position = new Point(x, y), AttackPower = 3 };
            ...
            while (Round()) {
                ...
                if (!fastForward && pDebug) {
                    ...
                    if (key.Key == ConsoleKey.P) { play = true; fastForward = true; }
                }
            };
            ...
            if (pExpectedTotal == 0) {
                Console.WriteLine(pFile + $": Rounds={rounds}, Hitpoints={sum}, Total={total}");
            } else {
