...
namespace day20_a_regular_map {
    class Part02 {
        ...
        static void Excavate(Path pPath) {
            ...
            var atleast1000Doors = Explore(start);

            //SaveMap(path);
            //DrawMap();
            Console.WriteLine();
            Console.WriteLine();
            Console.WriteLine((atleast1000Doors - 1) + ". Done!");
            Console.ReadKey(true);
        }

        static int Explore(Point pStart) {
            ...
            return deadEndDistance.Where(de => de.Value.Count >= 1000).Count();
        }
